...
class curation_system:
    ...
    def __init__(self, n_gpu, datatype, uid=None, output_path=None, enable_simul=None, start_posz=None, start_negz=None):
        self.n_gpu = n_gpu
        ...
        elif datatype == 'face_classfier_only':
            cfg = tf.ConfigProto()
            cfg.allow_soft_placement=True
            cfg.gpu_options.allow_growth=True
            self.sess = tf.InteractiveSession(config=cfg)
    ...
    def test_by_images(self, images):
        cnt = len(images)
        images = [cv2.resize(images[i], (160,160)) for i in range(cnt)]
        ...
        for i in range(fullrun):
            ...
            result[i*32:(i+1)*32] = np.transpose(rbatch)
        if remain > 0:
            featurebatch = np.zeros([32, featuremap_size])
